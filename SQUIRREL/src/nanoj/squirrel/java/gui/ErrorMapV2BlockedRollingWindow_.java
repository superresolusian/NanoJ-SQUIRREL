 ...
 */
public class ErrorMapV2BlockedRollingWindow_ extends _BaseDialog_ {
    ...
    public void execute() throws InterruptedException, IOException {
        ...
        ImageStack imsUsage = new ImageStack(w_SR, h_SR, nSlicesSR);
        ...
        for (int s = 0; s < nSlicesSR; s++) {
            ...
            float[] alphaValues = new float[nPixelsSR];
            float[] betaValues = new float[nPixelsSR];
            float[] sigmaValues = new float[nPixelsSR];


            log.status("getting parameters");
            NanoJThreadExecutor NTE = new NanoJThreadExecutor(false);

            for (int y = 0; y < h_SR; y++) {
                for (int x = 0; x < w_SR; x++) {
                    NTE.execute(new CalculateParametersBlock(fpSR, fpRef, x, y, blockSize, maxSigmaBoundary,
                            alphaValues, betaValues, sigmaValues));
                }
            }

            NTE.finish();

            //TODO: check for dodgy alpha values;

            FloatProcessor fpAlphaMap = new FloatProcessor(w_SR, h_SR, alphaValues);
            FloatProcessor fpBetaMap = new FloatProcessor(w_SR, h_SR, betaValues);
            FloatProcessor fpSigmaMap = new FloatProcessor(w_SR, h_SR, sigmaValues);

            float[] pixelsSRIntensityScaled = new float[nPixelsSR];

            for (int i = 0; i < nPixelsSR; i++) {
                pixelsSRIntensityScaled[i] = pixelsSR[i] * alphaValues[i] + alphaValues[i];
            }
            ...
            //local sigma convolution
            float[] timesUsed = new float[nPixelsSR];
            float[] pixelsConvolved = new float[nPixelsSR];

            NTE = new NanoJThreadExecutor(false);

            log.status("convolving");
            for (int y = 0; y < h_SR; y++) {
                for (int x = 0; x < w_SR; x++) {
                    float sigma = fpSigmaMap.getf(x, y);
                    NTE.execute(new ConvolvePerPixel(fpSRIntensityScaled, x, y,
                            sigma, pixelsConvolved, timesUsed));
                }
            }

            FloatProcessor fpUsage = new FloatProcessor(w_SR, h_SR, timesUsed);
            FloatProcessor fpSRConvolved = new FloatProcessor(w_SR, h_SR, pixelsConvolved);
            ...
            imsUsage.setProcessor(fpUsage, s+1);
        ...
        new ImagePlus("Usage", imsUsage).show();
    ...
    FloatProcessor getBlockFp(FloatProcessor fp, int x, int y, int blockSize){
        int w = fp.getWidth();
        int h = fp.getHeight();

        int blockRadius = blockSize/2;

        int xStart = Math.max(0, x-blockRadius);
        int yStart = Math.max(0, y-blockRadius);
        int xEnd = Math.min(w-1, x+blockRadius);
        int yEnd = Math.min(h-1, y+blockRadius);

        Rectangle rectangle = new Rectangle(xStart, yStart, xEnd-xStart+1, yEnd-yStart+1);
        fp.setRoi(rectangle);
    ...
    class CalculateParametersBlock extends Thread{
        private final FloatProcessor fpSR;
        private final FloatProcessor fpRef;
        private final int x;
        private final int y;
        ...
        public CalculateParametersBlock(FloatProcessor fpSR, FloatProcessor fpRef, int x, int y,
        ...
        private FloatProcessor getBlock(FloatProcessor fp, int blockRadius){
            int w = fp.getWidth();
            int h = fp.getHeight();
        ...
        public void run(){
            ...
            int blockRadiusRef = (int) blockSize/2;
            int blockRadiusSR = blockRadiusRef*magnification;

            FloatProcessor fpSRBlock = getBlock(fpSR, blockRadiusSR);
            FloatProcessor fpRefBlock = getBlock(fpRef, blockRadiusRef);
            float[] pixelsRefBlock = (float[]) fpRefBlock.getPixels();
            ...
            int blockWidthRef = fpRefBlock.getWidth(); //TODO: enforce that this is SR/mag
            int blockHeightRef = fpRefBlock.getHeight();
            ...
            /// setup optimizer
            sigmaOptimiseFunction f =  new ErrorMapV2BlockedRollingWindow_.sigmaOptimiseFunction(fpSRBlock, fpRefBlock, ones);
            ...
            alphas[p] = alphaBoundary;
            betas[p] = betaBoundary;
            sigmas[p] = sigmaBoundary;
        }

    }

    class ConvolvePerPixel extends Thread {
        private final FloatProcessor fpSR;
        private final int x;
        private final int y;
        private final float sigma;
        private final float[] convolvedPixels;
        private final float[] timesUsed;

        private final int w;
        private final int h;

        public ConvolvePerPixel(FloatProcessor fpSR, int x, int y, float sigma,
                                float[] convolvedPixels, float[] timesUsed) {
            this.fpSR = fpSR;
            this.x = x;
            this.y = y;
            this.sigma = sigma;
            this.convolvedPixels = convolvedPixels;
            this.timesUsed = timesUsed;

            this.w = fpSR.getWidth();
            this.h = fpSR.getHeight();
        }

        public void run(){

            int blockSize = (int) (10*sigma);
            int blockRadius = blockSize/2;
            ...
            Rectangle rectangle = new Rectangle(xStart, yStart, xEnd-xStart+1, yEnd-yStart+1);
            fpSR.setRoi(rectangle);

            FloatProcessor fpCrop = fpSR.crop().convertToFloatProcessor();
            fpSR.resetRoi();

            fpCrop.blurGaussian(sigma);

            for(int y_=yStart; y_<=yEnd; y_++){
                for(int x_=xStart; x_<=xStart; x_++){

                    int p = y_*w + x_;

                    convolvedPixels[p] += fpCrop.getf(x_-xStart, y_-yStart);
                    timesUsed[p]++;
